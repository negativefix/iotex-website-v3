...
var require_queue_microtask = __commonJS((exports, module) => {
  ...
  var promise;
  module.exports = typeof queueMicrotask === "function" ? queueMicrotask.bind(globalThis) : (cb) => (promise || (promise = Promise.resolve())).then(cb).catch((err) => setTimeout(() => {
    throw err;
  }, 0));
});
...
function log(string, color = "red") {
  return console.log(_chalk2.default.keyword(color)(`
		[vite-aliases]: ${string}
	`));
}
function createLogfile(path, data) {
  _fs.mkdir.call(void 0, `${path}`, {recursive: true}, (error) => {
    _fs.writeFile.call(void 0, `${path}/vite-aliases.json`, JSON.stringify(data), (error2) => {
      if (error2) {
...
function getDirectories(options) {
  ...
  const directories = import_fast_glob.default.sync(deep ? `${path}/**/*` : `${path}/*`, {
    ignore: ["node_modules"],
...
function getAliases(options = {}) {
  ...
  const guide = addLeadingSlash ? `/${prefix}` : prefix;
  const directories = getDirectories({path, deep, root, depth});
  const aliases = directories.map((path2) => {
    const dir = split(path2, "/").slice(-1)[0];
    return {
      find: `${guide}${dir}`,
      replacement: _slash2.default.call(void 0, `${root}/${path2}`)
    };
  });
  ...
  if (allowGlobalAlias) {
    aliases.push({
      find: `${guide}`,
      replacement: _slash2.default.call(void 0, `${root}/${path}`)
    });
  }
  if (allowLogging) {
